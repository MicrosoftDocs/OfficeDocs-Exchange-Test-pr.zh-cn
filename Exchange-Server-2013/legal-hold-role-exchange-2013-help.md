---
title: '法定保留角色: Exchange 2013 Help'
TOCTitle: 法定保留角色
ms:assetid: c98ce8ca-3477-479a-ad23-a8e6459bc4d0
ms:mtpsurl: https://technet.microsoft.com/zh-cn/library/Dd876939(v=EXCHG.150)
ms:contentKeyID: 50491687
ms.date: 05/21/2018
...
`Legal Hold` 管理角色使管理员能够配置是否应在组织中保留邮箱中的数据以用于诉讼目的。
...
除了收件人范围， **Enable-Mailbox** cmdlet，包含与该角色，则也被范围使用数据库配置作用域。数据库配置作用域控制该 cmdlet 可以在中创建新邮箱的数据库。要创建邮箱的数据库必须在数据库范围内。这适用于指定**Enable-Mailbox** cmdlet，使用*Database*参数的数据库时，或如果允许自动邮箱分发，以供您选择的数据库。有关详细信息，请参阅[了解管理角色作用域](understanding-management-role-scopes-exchange-2013-help.md)。
