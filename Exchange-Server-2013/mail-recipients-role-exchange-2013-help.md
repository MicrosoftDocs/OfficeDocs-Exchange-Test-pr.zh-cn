---
title: '邮件收件人角色: Exchange 2013 Help'
TOCTitle: 邮件收件人角色
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/zh-cn/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 50490898
ms.date: 01/11/2018
...
`Mail Recipients` 管理角色使管理员能够管理组织中的现有邮箱、邮件用户和邮件联系人。此角色无法创建这些收件人。可使用 `Mail Recipient Creation` 角色进行创建。

无法使用此类角色管理启用了邮件功能的公用文件夹或通讯组。可使用下列角色管理这些对象：

  - [启用邮件的公用文件夹角色](mail-enabled-public-folders-role-exchange-2013-help.md)

  - [通讯组角色](distribution-groups-role-exchange-2013-help.md)

如果您的组织使用拆分权限模型，即收件人的创建和管理由不同组执行，请将 `Mail Recipient Creation` 角色分配给创建收件人的组，将 `Mail Recipients` 角色分配给管理收件人的组。有关详细信息，请参阅下列主题：
...
除收件人作用域外，**Connect-Mailbox** 和 **Enable-Mailbox** cmdlet（包含于此角色中）也可以使用数据库配置作用域设置其作用域。数据库配置作用域控制 cmdlet 可以在哪些数据库上创建新邮箱。您要在其中创建邮箱的数据库必须在数据库作用域内。当您使用 *Database* 参数在上述任一 cmdlet 上指定数据库时，或者如果允许自动邮箱分发为您选择数据库，需要遵循此条件。有关详细信息，请参阅[了解管理角色作用域](understanding-management-role-scopes-exchange-2013-help.md)。
