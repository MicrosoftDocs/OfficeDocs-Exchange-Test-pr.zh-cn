---
title: '启用邮件的公用文件夹角色: Exchange 2013 Help'
TOCTitle: 启用邮件的公用文件夹角色
ms:assetid: 22289ae1-f9e8-4c98-aaad-ee623e05eadd
ms:mtpsurl: https://technet.microsoft.com/zh-cn/library/Dd876859(v=EXCHG.150)
ms:contentKeyID: 50490205
ms.date: 05/21/2018
...
`Mail Enabled Public Folders` 管理角色使管理员能够将组织中的单个公用文件夹配置为启用邮件或禁用邮件。

此角色可以管理公用文件夹的电子邮件属性。它不会使您能够管理不相关的电子邮件的公用文件夹属性。若要管理公用文件夹属性不相关的电子邮件，请使用`Public Folders`角色。有关详细信息，请参阅[公用文件夹角色](public-folders-role-exchange-2013-help.md)。
