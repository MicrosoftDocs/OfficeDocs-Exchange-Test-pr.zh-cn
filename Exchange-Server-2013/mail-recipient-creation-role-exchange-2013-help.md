---
title: '邮件收件人创建角色: Exchange 2013 Help'
TOCTitle: 邮件收件人创建角色
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/zh-cn/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50490940
ms.date: 05/21/2018
...
`Mail Recipient Creation` 管理角色使管理员能够在组织中创建邮箱、邮件用户、邮件联系人、通讯组和动态通讯组。此角色可与 `Mail Recipients` 角色结合使用，以便能够创建和管理收件人。有关详细信息，请参阅[邮件收件人角色](mail-recipients-role-exchange-2013-help.md)。

此角色不允许您对公用文件夹启用邮件。若要对公用文件夹启用邮件，则必须使用`Mail Enabled Public Folders`角色。有关详细信息，请参阅[启用邮件的公用文件夹角色](mail-enabled-public-folders-role-exchange-2013-help.md)。

如果在组织维护的基于角色的访问控制 (RBAC) 拆分权限模型中，执行收件人创建的组不同于执行收件人管理的组，则将`Mail Recipient Creation`角色分配给执行收件人创建的管理角色组，将`Mail Recipients`角色分配给执行收件人管理的角色组。
...
有关 RBAC 和 Active Directory 拆分权限的详细信息，请参阅[了解拆分权限](understanding-split-permissions-exchange-2013-help.md)。
...
除收件人作用域之外，还使用数据库配置作用域设置了包含在此角色中的 **New-Mailbox** cmdlet 的作用域。数据库配置作用域控制 cmdlet 能在哪些数据库上创建新的邮箱。您要在其中创建邮箱的数据库必须在数据库作用域内。当您使用 **New-Mailbox** cmdlet 上的 *Database* 参数指定数据库，或允许自动邮箱分配为您选择数据库时，该条件适用。有关详细信息，请参阅[了解管理角色作用域](understanding-management-role-scopes-exchange-2013-help.md)。
