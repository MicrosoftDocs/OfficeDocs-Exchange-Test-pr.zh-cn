...
设置 ASA 凭据时，请牢记这些准则：
  ...
  - <strong>帐户类型。</strong>我们建议您创建计算机帐户，而非用户帐户。计算机帐户不允许交互式登录，且安全策略可能比用户帐户简单。如果您创建了计算机帐户，实际上密码并不会过期，但是我们仍然建议您定期更新密码。您可以使用本地组策略指定计算机帐户和脚本的最长期限，达到此期限后，将定期删除不符合当前策略的计算机帐户。您的本地安全策略也决定着何时需要更改密码。我们建议您使用计算机帐户，但您也可以创建用户帐户。
